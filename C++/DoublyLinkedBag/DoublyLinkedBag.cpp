...
//Rule of 5
    List() = default; 
    List(const List& o) { copyFrom(o); } //Copy constructor
    List(List&& o) noexcept : head(o.head), tail(o.tail) { //move constructor
        o.head = o.tail = nullptr;
    }
    List& operator=(const List& o) { //Assignment
        if (this != &o) {
            clear();
            copyFrom(o);
        }
    ...
    List& operator=(List&& o) noexcept { //move assignment
        if (this != &o) {
            clear();
    ...
    void traverse(void (*doIt)(const T&)) const {
